...
Assume now that the statement does not hold for any $\eps<0$.
Then we must have inequalty in \ref{eq:area=<a} cannot be improved to \[\area (\cont K_s)\le \frac{n\cdot\omega_n}{\omega_{n-1}}\cdot a(\cont K_s)-\delta;\] for fixed $\delta>0$ and all small $s>0$.
It follows that $\cont K=\partial(\conv K)$, and the last statement follows.
\end{proof}
