...
The Favard area is well-defined. Indeed, the dependence of the projection on the hyperplane in this case has closed graph. Therefore, Fubini's theorem implies the integrability of the area of the projection as a function of the hyperplane.
...
Moreover, the same holds for some $\eps<0$ unless $\partial K\supset \partial(\conv K)$;
here $\conv K$ denotes the convex hull of $K$.
\end{thm}

\begin{proof}
Let us denote by $K_s$ the closed $s$-neighborhood of $K$. Good thing about $K_s$ is that the convex hull $\conv K_s$ has $C^{1,1}$-smooth boundary; in particular, its second derivatives exist almost everywhere and are bounded.
...
Let us now discuss the improved bound with $\eps < 0$. If the inequality in \ref{eq:area=<a} can be improved to
\[
\area (\cont K_s)\le \frac{n\cdot\omega_n}{\omega_{n-1}}\cdot a(\cont K_s)-\delta,
\] 
for some fixed $\delta>0$ and all sufficiently small $s>0$ then the final estimate can be extended to some $\eps<0$. Otherwise, for a sequence $s_k\to+0$ we have the gap in \ref{eq:area=<a} tend to zero. \textbf{If we could assert that $\area(\cont K_s)\to \area(\cont K)$ and $a(\cont K_s)\to a(\cont K)$ then the equality in the Crofton estimate \ref{eq:area=<a} for $s=0$ and $K_0=K$ would imply that almost every line intersecting $\cont K$ intersects it twice. From the compactness of $\cont K$ this would imply} $\cont K=\partial(\conv K)$, and the last statement would follow.
\end{proof}
...
$J_w(u,r)$ denotes the Jacobian at $(u,r)$ of the projection onto $w^\perp$ of the restriction $\xi|_{\SSS^{n-2}\times \RR}$. This estimate holds true since the points with $\phi(u,r)=1$ are projected injectively to $L$ with the Jacobian $J_w$ and do not get into $L_t$.
...
\]
Let us explain this in detail. We know that
\[
...
%\]
Using $\vol B=\tfrac{\omega_{n}}{\kappa(K)^{n}}=\tfrac{\omega_n a(K)^{n/(n-1)}}{\omega_{n-1}^{n/(n-1)}}$) and considering the derivatives of $s_\delta(K)$ as a function of $\vol$ and $a$, one observes that
\[
\frac{\partial s_\delta(K)}{\partial\vol} = 1,\quad \frac{\partial s_\delta(K)}{\partial a} = (1+\delta)\frac{n\omega_n}{(n-1)\omega_{n-1}^{n/(n-1)}} a(K)^{1/(n-1)}
\]
