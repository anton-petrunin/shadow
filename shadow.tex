...
By local Steiner's formula \cite[4.2.1]{schneider}, $s\mapsto \area (\cont K_s)$ is a polynomial function, so it must be continous at $s= 0$.
Furthermore, $s\mapsto a(\cont K_s)$ is upper semicontinuous at $s= 0$.
It follows that we can improve \ref{eq:area=<a} to
\[\area (\cont K_s)\le \frac{n\cdot\omega_n}{\omega_{n-1}}\cdot a(\cont K_s)-\delta,\]
for some fixed $\delta>0$ and all sufficiently small $s>0$.
