...
By Cauchy's projection formula, this theorem gives a more exact version of the standard isoperimetric inequality.%
\footnote{Это следует из вот этих неравенств:
\[2\cdot a(\partial K)\le \mathcal{I}^{n-1}_1(\partial K)\le \mathcal{H}^{n-1}(\partial K),\]
здесь $\mathcal{I}^{n-1}_1$ --- integralgeometric measure.
Второе неравенство доказывается разбиением множества конечной меры на спрямляемое и полностью неспрямляемое \cite[15.6]{mattila} + \cite[3.2.26]{federer}.}
...
\begin{proof}
Let $u\in \SSS^{n-1}$ and $r\in \RR$. Consider the unique point $\bar x(u)\in \partial C$ with outer normal vector $u$.
Then the map
\[
