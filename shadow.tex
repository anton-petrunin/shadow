...
By a Crofton-like formula \cite[3.3.13]{federer}, this theorem gives a more exact version of the standard isoperimetric inequality.
For the equality case, we gave only a necessary condition, which is not sufficient.
Equality holds for the union of a ball with any set of vanishing Favard area,
...
Moreover, if $K$ is not convex, then the same holds for some $\eps<0$.
\textbf{This has no proof, see below.}
\end{thm}
...
Finally, note that if $K$ is not convex, then inequality in \ref{eq:area=<a} is strict.
\textbf{This is false. For example, if $K$ is convex minus something in its interior then $K_s$ is of the same kind with equality in \ref{eq:area=<a}. Of course, from compactness of $K$ in this case one may replace $K$ by its convex hull increasing its volume and not changing $a(K)$.}  

This leads to the last statement.
\textbf{No, after \ref{eq:area=<a} there is an implicit passing to the limit $s\to+0$ with $\kappa(K_s)\to\kappa(K)$ most probably from above. In the statement of the lemma $\eps<0$ is assumed the same for all $s$ while the difference in the strict inequality \ref{eq:area=<a} must definitely depend on $s$ and may not produce a negative $\eps$ in the limit.}
\end{proof}
...
\textbf{Here must be the formulas.}
...
By the outer regularity of the Lebesgue measure there exists an open $U\supset M$ whose volume is arbitrarily close to $\vol M$.
...
We need to show that $K$ is congruent to $B$.
If $K$ is convex, then it follows from the equality case in the standard isoperimetric inequality.
